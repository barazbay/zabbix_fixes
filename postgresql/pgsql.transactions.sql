	...
	IF (ver >= 90600) THEN
		SELECT row_to_json(T) INTO res from (
			SELECT
				...
				coalesce(extract(epoch FROM max(CASE WHEN state = 'active' AND state <> 'idle' THEN age(now(), xact_start) END)), 0) AS active,
	...
	ELSE
		SELECT row_to_json(T) INTO res from (
			SELECT
				coalesce(extract(epoch FROM max(CASE WHEN state = 'idle in transaction' THEN age(now(), xact_start) END)), 0) AS idle,
				coalesce(extract(epoch FROM max(CASE WHEN state = 'active' AND state <> 'idle' THEN age(now(), xact_start) END)), 0) AS active,
				coalesce(extract(epoch FROM max(CASE WHEN waiting IS TRUE THEN age(now(), xact_start) END)), 0) AS waiting,
				(SELECT coalesce(extract(epoch FROM max(age(now(), prepared))), 0) FROM pg_prepared_xacts) AS prepared
			FROM pg_stat_activity) T;
	END IF;
